<?php
/*
Plugin Name: Subscribe To Comments for Mondo
Version: 1.0
Description: This is a slight modification of Subscribe to Comments plugin by Mark Jaquith. It allows readers to choose to receive notifications of new comments that are posted to an entry. Removing subscription management links.
Author: Yoshie
*/
...
if ( !_stc()->current_viewer_subscription_status() ) :
	...
	<div class="solo-subscribe-to-comments">
		<p><?php _e( 'Subscribe for comment updates', 'subscribe-to-comments' ); ?></p>
		<label for="solo-subscribe-email"><?php _e( 'Email:', 'subscribe-to-comments' ); ?>
		<input type="text" name="email" id="solo-subscribe-email" size="22" value="<?php echo esc_attr( $user_email ); ?>" /></label>
		<input type="submit" class="button" name="submit" value="<?php _e( 'Subscribe', 'subscribe-to-comments' ); ?>" />
	</div>
	</form>
...
class CWS_STC {
	...
	function register_hooks() {
		...
		*/
		...
		//COMMENTING THIS OUT SO WE CAN MANUALLY INSERT WHEN WE WANT TO SEE THE CHECKBOX OPTION
...
<?php
	...
	function solo_subscribe ( $email, $postid ) {
		...
		if ( ( $email == $this->site_email && is_email( $this->site_email ) ) || (  $email == get_option( 'admin_email' ) && is_email( get_option( 'admin_email' ) ) ) )
			$this->add_error( __( 'This email address may not be subscribed', 'subscribe-to-comments' ),'solo_subscribe' );
	...
	function change_email_request() {
		...
		$subject = __( 'Email change confirmation', 'subscribe-to-comments' );
		$message = sprintf( __( "You are receiving this message to confirm a change of email address for your subscriptions at \"%s\"\n\n", 'subscribe-to-comments' ), get_bloginfo( 'blogname' ) );
		$message .= sprintf( __( "To change your email address to %s, click this link:\n\n", 'subscribe-to-comments' ), $this->new_email );
	...
	function block_email_request( $email ) {
		if ( $this->is_blocked( $email ) )
			return false;
		$subject = __( 'Email block confirmation', 'subscribe-to-comments' );
		$message = sprintf( __( "You are receiving this message to confirm that you no longer wish to receive email comment notifications from \"%s\"\n\n", 'subscribe-to-comments' ), get_bloginfo( 'name' ) );
...
function options_page_contents() {
	...
	echo '<h3>' . __( 'Notification emails', 'subscribe-to-comments' ) . '</h3>';
...
function options_page() {
	...
	echo '<p class="submit"><input type="submit" class="button" name="sg_subscribe_settings_submit" value="';
		...
		case "email_change_request" :
			if ( _stc()->is_blocked( _stc()->email ) )
				...
				if ( _stc()->change_email_request( _stc()->email, _stc()->new_email ) )
					_stc()->add_message( sprintf( __( 'Your change of email request was successfully received.  Please check your old account (<strong>%s</strong>) in order to confirm the change.', 'subscribe-to-comments' ), _stc()->email ) );
			break;

		case "block_request" :
			if ( _stc()->block_email_request( _stc()->email ) )
				_stc()->add_message( sprintf( __( 'Your request to block <strong>%s</strong> from receiving any further notifications has been received.  In order for you to complete the block, please check your email and click on the link in the message that has been sent to you.', 'subscribe-to-comments' ), _stc()->email ) );
			break;
...
?>
	...
	<?php _stc()->hidden_form_fields(); ?>
			...
			<p class="submit">
			<input type="submit" class="button" name="submit" value="<?php _e( 'Remove Block &raquo;', 'subscribe-to-comments' ); ?>" />
			</p>
			</form>
...
<?php
		if ( isset( _stc()->email ) && !is_array( $postlist ) && _stc()->email != _stc()->site_email && _stc()->email != get_bloginfo( 'admin_email' ) ) {
			if ( is_email( _stc()->email ) )
				...
				_stc()->add_error( sprintf( __( '<strong>%s</strong> is not a valid email address.', 'subscribe-to-comments' ), _stc()->email ) );
		}
?>
	...
	<?php if ( current_user_can( 'manage_options' ) ) { ?>
			...
			<p>
			<?php _e( 'Enter an email address to view its subscriptions or undo a block.', 'subscribe-to-comments' ); ?>
			</p>
			...
			<input type="submit" class="button-secondary button" value="<?php _e( 'Search &raquo;', 'subscribe-to-comments' ); ?>" />
			</p>
			</form>
...
<?php elseif ( !isset( $_REQUEST['showccfield'] ) || !$_REQUEST['showccfield'] ) : ?>
	<p><a href="<?php echo add_query_arg( 'showccfield', '1' ); ?>"><?php _e( 'Show list of subscribers in <code>CC:</code>-field format (for bulk emailing)', 'subscribe-to-comments' ); ?></a></p>
<?php else : ?>
...
//-->
	...
	<!-- <?php _stc()->hidden_form_fields(); ?> -->
				...
				<p class="submit">
				<input type="submit" class="button" name="submit" value="<?php _e( 'Remove Selected Subscription(s) &raquo;', 'subscribe-to-comments' ); ?>" />
				</p>
				</form>
	...
	<?php _stc()->hidden_form_fields(); ?>
				...
				<p class="submit">
				<input type="submit" class="button" name="submit" value="<?php _e( 'Block Notifications &raquo;', 'subscribe-to-comments' ); ?>" />
				</p>
				</form>



			<h3><?php _e( 'Change email Address', 'subscribe-to-comments' ); ?></h3>
	...
	<?php _stc()->hidden_form_fields(); ?>
				...
				<p>
				<?php printf( __( 'If you would like to change the email address for your subscriptions, enter the new address below.  You will be required to verify this request by clicking a special link sent to your current address (<strong>%s</strong>).', 'subscribe-to-comments' ), _stc()->email ); ?>
				</p>

				<p>
				<?php _e( 'New Email Address:', 'subscribe-to-comments' ); ?>
				<input name="new_email" type="text" id="new_email" size="40" />
				<input type="submit" name="submit" class="button-secondary button" value="<?php _e( 'Change Email Address &raquo;', 'subscribe-to-comments' ); ?>" />
				</p>
				</form>
